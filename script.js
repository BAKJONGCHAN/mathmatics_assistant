...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // --- [보안 강화] 텍스트를 SHA-256 해시로 변환하는 함수 ---
    ...
    async function handleTeacherLogin() {
        ...
        const correctHash = 'a5b50198305764653f5b49764f69e6b4121b83a73c3335b0ac7a4f901a59f518';
    ...
    async function handleStudentLogin() {
        ...
        const correctHash = 'f9e0b1626388dd19f4e246835a828972f03f56b539a456952843792078a3c869';
        const inputHash = await sha256(password);
    ...
    async function startTutoringSession() {
        ...
        try {
            ...
            const initialPrompt = `당신은 고등학생을 위한AI 수학 튜터입니다. 당신의 목표는 학생이 답을 찾도록 안내하는 것입니다. [매우 중요한 규칙] 1. 모든 답변은 반드시 한국어로 해야 합니다. 2. 답변은 문장 단위로 줄을 바꾸어(개행하여) 명확하게 제시해주세요. 학생들이 한 번에 한 문장씩 집중해서 읽을 수 있도록 도와줍니다. 3. 필요시 정답을 알려주세요. 힌트나 다음 단계를 생각하게 하는 질문을 던지세요. 4. 수학 용어와 기호는 LaTeX 형식($...$)을 사용하여 명확하게 표시해주세요. 학생이 풀 문제는 다음과 같습니다: "${problemText}" [교사 요청사항]: ${localStorage.getItem('teacher-prompt')} 이제 위의 모든 규칙을 철저히 지키면서, 학생에게 튜터링을 시작해주세요.`;
    ...
    async function ocrChef(file) {
        ...
        const ocrPrompt = `당신은 수학 문제 전문 OCR 분석가입니다. 당신의 임무는 그림과 글이 섞인 복잡한 수학 문제 이미지에서 모든 텍스트 정보를 완벽하게 추출하는 것입니다. 다음의 '단계적 분석' 과정을 반드시 내부적으로 수행하여 정확도를 극대화하세요: 1.  **[1단계: 영역 분할]** 이미지 전체를 훑어보고, '문제 서술부', '핵심 수식부(예: lim)', '기하 도형부', '단서 조항부'로 시각적 영역을 나눕니다. 2.  **[2단계: 개별 텍스트 추출]** 각 영역의 텍스트를 하나씩, 매우 신중하게 읽어냅니다. 특히 '기하 도형부' 안에 포함된 작은 글씨나 기호(예: f(θ), g(θ), R, U, T, θ, 2θ 등)를 절대 놓치지 마세요. 3.  **[3단계: 종합 및 정리]** 1, 2단계에서 추출한 모든 정보를 모아, 논리적인 순서에 따라 하나의 완전한 문제 텍스트로 재구성합니다. 최종 결과물은 3단계에서 완성된, 깨끗하게 정리된 텍스트만 보여주세요. 당신의 생각 과정(1, 2단계)은 출력하지 마세요.`;
